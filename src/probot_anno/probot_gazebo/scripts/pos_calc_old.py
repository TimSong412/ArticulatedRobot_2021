...
class Pos_Calc:
    def __init__(self, l1=284, l2=225, l3=228.9, l4=0, t1_ini=-90, t2_ini=-90, t3_ini=0, t4_ini=0, t5_ini=0, t6_ini=0):
        self.l1 = l1
        self.l2 = l2
        self.l3 = l3
        self.l4 = l4
    ...
    def calc_pos(self, theta1, theta2, theta3, theta4, theta5, theta6):
        ...
        T03 = np.around(np.array([[c1*c2*c3 - c1*s2*s3, -s1, c1*c2*s3+c1*s2*c3, self.l2*c1*c2],
                        [s1*c2*c3 - s1*s2*s3, c1, s1*c2 *
                            s3+s1*s2*c3, self.l2*c2*s1],
                        [-s2*c3-s3*c2, 0, -s2*s3+c2*c3, -self.l2*s2+self.l1],
                        [0, 0, 0, 1]]),
                        self.acc)

        T36 = np.around(np.array([[c4*c5*c6 - s4*s6, -c6*s4 - c4*c5*s6, c4*s5, self.l4*c4*s5],
...
def main():
    ...
    # cos(rad)
    calculator = Pos_Calc()
    # res = calculator.calc_angle(0.0924592143592115, 0.03365240191039125, 0.6425649235340782, -0.4420150775729469, -0.3463237779149586, -2.0680349767815014)
    # print(res)
    # print(calculator.calc_pos(-45, 60, -45, 60, -45, 60))
    print(calculator.calc_pos(20, 20, 20, 20, 20, 20))
    # print(calculator.calc_pos(0, 0, 0, 0, 0, 0))
    print(calculator.calc_pos(-20.000000000000014, 24.664197194270606, -66.97750144851543, -14.281670103895822, 103.06225558932488, 70.1236007288458))
